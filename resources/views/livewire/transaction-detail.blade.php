            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @if($cpName)
                    <div>
                        ...
                        <dd class="text-sm font-medium text-gray-900 mt-0.5">{{ $cpName }}</dd>
                    </div>
                @endif
                @if($cpIban)
                    <div>
                        <dt class="text-[11px] text-gray-400">IBAN</dt>
                        <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $cpIban }}</dd>
                    </div>
                @endif
                @if($cpAgent)
                    <div>
                        <dt class="text-[11px] text-gray-400">BIC</dt>
                        <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $cpAgent }}</dd>
                    </div>
                @endif
            </div>
        </div>
                ...
                <div class="space-y-3">
                    @if($transaction->reference)
                        <div>
                            ...
                            <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->reference }}</dd>
                        </div>
                    @endif
                    @if($transaction->remittance_information)
                        <div>
                            <dt class="text-[11px] text-gray-400">Verwendungszweck</dt>
                            <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->remittance_information }}</dd>
                        </div>
                    @endif
                    @if($transaction->remittance_information_structured)
                        <div>
                            <dt class="text-[11px] text-gray-400">Strukturiert</dt>
                            <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->remittance_information_structured }}</dd>
                        </div>
                    @endif
                    @if($transaction->remittance_information_unstructured)
                        <div>
                            <dt class="text-[11px] text-gray-400">Unstrukturiert</dt>
                            <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->remittance_information_unstructured }}</dd>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if($transaction->additional_information)
                        <div class="md:col-span-2">
                            ...
                            <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->additional_information }}</dd>
                        </div>
                    @endif
                    @if($transaction->additional_information_structured)
                        <div class="md:col-span-2">
                            <dt class="text-[11px] text-gray-400">Strukturierte Zusatzinfo</dt>
                            <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->additional_information_structured }}</dd>
                        </div>
                    @endif
                    @if($transaction->purpose_code)
                        <div>
                            <dt class="text-[11px] text-gray-400">Purpose Code</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->purpose_code }}</dd>
                        </div>
                    @endif
                    @if($transaction->end_to_end_id)
                        <div>
                            <dt class="text-[11px] text-gray-400">End-to-End ID</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->end_to_end_id }}</dd>
                        </div>
                    @endif
                    @if($transaction->mandate_id)
                        <div>
                            <dt class="text-[11px] text-gray-400">Mandatsreferenz</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->mandate_id }}</dd>
                        </div>
                    @endif
                    @if($transaction->merchant_category_code)
                        <div>
                            <dt class="text-[11px] text-gray-400">Merchant Category Code</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->merchant_category_code }}</dd>
                        </div>
                    @endif
                    @if($transaction->creditor_id)
                        <div>
                            <dt class="text-[11px] text-gray-400">Gläubiger-ID</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->creditor_id }}</dd>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if($transaction->transaction_id)
                        <div>
                            ...
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->transaction_id }}</dd>
                        </div>
                    @endif
                    @if($transaction->internal_transaction_id)
                        <div>
                            <dt class="text-[11px] text-gray-400">Internal Transaction ID</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->internal_transaction_id }}</dd>
                        </div>
                    @endif
                    @if($transaction->entry_reference)
                        <div>
                            <dt class="text-[11px] text-gray-400">Entry Reference</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->entry_reference }}</dd>
                        </div>
                    @endif
                    @if($transaction->bank_transaction_code)
                        <div>
                            <dt class="text-[11px] text-gray-400">Bank Transaction Code</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->bank_transaction_code }}</dd>
                        </div>
                    @endif
                    @if($transaction->proprietary_bank_transaction_code)
                        <div>
                            <dt class="text-[11px] text-gray-400">Proprietary Code</dt>
                            <dd class="text-sm font-mono text-gray-700 mt-0.5">{{ $transaction->proprietary_bank_transaction_code }}</dd>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <x-ui-page-sidebar title="Details" width="w-80" side="right" :defaultOpen="true" storeKey="activityOpen">
            <div class="p-4 space-y-4">
                <div>
                    ...
                    <dd class="text-sm font-medium text-gray-900 mt-0.5">{{ $transaction->bankAccount->name ?? '-' }}</dd>
                </div>
                ...
                <div>
                    ...
                    <dd>
                        <select wire:model.live="categoryId"
                                class="w-full px-2 py-1 rounded-md border border-gray-200 text-sm text-gray-900 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->status ?? '-' }}</dd>
                </div>

                <div>
                    <dt class="text-[11px] text-gray-400 uppercase tracking-wide">Währung</dt>
                    <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->currency ?? '-' }}</dd>
                </div>
                ...
                <div>
                    <button wire:click="createBudgetFromTransaction"
                            class="w-full inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                ...
                <div>
                    ...
                    <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->booking_date?->format('d.m.Y') ?? $transaction->booked_at?->format('d.m.Y') ?? '-' }}</dd>
                </div>

                <div>
                    <dt class="text-[11px] text-gray-400 uppercase tracking-wide">Wertstellungsdatum</dt>
                    <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->value_date?->format('d.m.Y') ?? '-' }}</dd>
                </div>
                ...
                <div>
                    ...
                    <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->created_at?->format('d.m.Y H:i') ?? '-' }}</dd>
                </div>

                <div>
                    <dt class="text-[11px] text-gray-400 uppercase tracking-wide">Aktualisiert</dt>
                    <dd class="text-sm text-gray-700 mt-0.5">{{ $transaction->updated_at?->format('d.m.Y H:i') ?? '-' }}</dd>
                </div>
            </div>
        </x-ui-page-sidebar>
    </x-slot>
