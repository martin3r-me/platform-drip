<div class="space-y-8">
    ...
    <x-ui.modal wire:model="showInstitutionModal">
        ...
        <div class="space-y-4">
            <x-ui.input label="Name" wire:model.defer="institutionForm.name" />
            <x-ui.input label="Land (ISO-2)" wire:model.defer="institutionForm.country" />
            <x-ui.input label="Externe ID" wire:model.defer="institutionForm.external_id" />
        </div>
        <x-slot:footer>
    ...
    <x-ui.modal wire:model="showGroupModal">
        ...
        <div class="space-y-4">
            <x-ui.input label="Name" wire:model.defer="groupForm.name" />
            <x-ui.input label="Farbe" wire:model.defer="groupForm.color" />
        </div>
        <x-slot:footer>
    ...
    <x-ui.modal wire:model="showAccountModal">
        ...
        <div class="space-y-4">
            <x-ui.input label="Name" wire:model.defer="accountForm.name" />
            <x-ui.input label="IBAN" wire:model.defer="accountForm.iban" />
            <x-ui.input label="Währung" wire:model.defer="accountForm.currency" />
            <x-ui.select label="Bank" wire:model.defer="accountForm.institution_id">
                <option value="">—</option>
                @foreach ($institutions as $i)
                    <option value="{{ $i->id }}">{{ $i->name }}</option>
                @endforeach
            </x-ui.select>
            <x-ui.select label="Gruppe" wire:model.defer="accountForm.group_id">
                <option value="">—</option>
                @foreach ($groups as $g)
