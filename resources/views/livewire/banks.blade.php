<x-ui-page>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 mb-8">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h2 class="text-base font-semibold text-gray-900">Banken verbinden</h2>
                <p class="text-sm text-gray-500 mt-0.5">Verbinde deine Bank über GoCardless</p>
            </div>
        ...
        @if (!empty($gocardlessInstitutions))
            <div class="mb-4">
                <input type="text" wire:model.live="search" placeholder="Bank suchen..."
                       class="w-full px-3 py-1.5 rounded-md border border-gray-200 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
            </div>
                        ...
                        <div class="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-sm transition-shadow">
                            <div class="flex items-center space-x-3 mb-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h3 class="text-sm font-semibold text-gray-900 truncate">{{ $bank['name'] }}</h3>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <p class="text-sm text-gray-500">Keine passenden Banken gefunden.</p>
                </div>
            @endif
        @else
            <div class="text-center py-12 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <h3 class="text-base font-semibold text-gray-900 mb-1">Banken verbinden</h3>
                <p class="text-sm text-gray-500">Lade verfügbare Banken, um deine Konten zu verbinden.</p>
            </div>
        @endif
    </div>
    ...
    <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500 mt-0.5">{{ $group->accounts->count() }} Konten</p>
                    </div>
                ...
                @else
                    <div class="text-center py-8 bg-gray-50 rounded-xl border border-gray-100">
                        ...
                        <p class="text-sm text-gray-500">Keine Konten in dieser Gruppe</p>
        ...
        @empty
            <div class="bg-white rounded-xl border border-gray-200 p-12 text-center">
                ...
                <h3 class="text-base font-semibold text-gray-900 mb-1">Keine Kontogruppen</h3>
                <p class="text-sm text-gray-500 mb-4">Erstelle eine Kontogruppe, um deine Bankkonten zu organisieren.</p>
                <button type="button" wire:click="openGroupModal"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-green-600 text-white text-[13px] font-medium hover:bg-green-700 transition-colors">
                    @svg('heroicon-o-plus', 'w-4 h-4')
    ...
    @if ($accounts->whereNull('group_id')->count() > 0)
        <div class="bg-white rounded-xl border border-gray-200 p-6 mt-8">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h3 class="text-base font-semibold text-gray-900">Konten ohne Gruppe</h3>
                    <p class="text-sm text-gray-500 mt-0.5">{{ $accounts->whereNull('group_id')->count() }} Konten warten auf Zuordnung</p>
                </div>
            </div>
    ...
    <x-ui-modal model="showGroupSelectionModal">
        ...
        <div class="space-y-4">
            <p class="text-sm text-gray-500">Wähle eine Gruppe für das Konto aus:</p>
            <div class="grid grid-cols-1 gap-3">
                @foreach ($groups as $group)
                    <button
                        type="button"
                        wire:click="assignToGroup({{ $group->id }})"
                        class="flex items-center justify-between p-4 border border-gray-200 rounded-xl hover:bg-blue-50/50 hover:border-gray-300 transition-colors"
                    >
